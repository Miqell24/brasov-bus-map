...
// Brașov: ONE RATBV feed split by route_type — buses and trolleybuses (route_type 3 and 11) on the
// road graph. Trolleybuses ride the bus network in green
...
// ONE feed, ONE cfg: routeTypes splits the RATBV bundle per mode
...
// Brașov has no metro, so the third cfg its siblings carry is absent here.
const MODES = [{
  mode: 'bus', label: 'buses', osmFile: 'data/osm/brasov.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['23']),
  feeds: [
    {
      tag: 'brasov', dir: 'data/gtfs', routeTypes: ['3', '11'],
      mapKey: (sn) => norm(sn),
    },
  ],
}];
...
async function processMode(cfg) {
  ...
  for (const feed of cfg.feeds) {
    ...
    for (const s of await readCsv(join(fdir, 'stops.txt'))) {
      ...
      // RATBV publishes its stop names properly cased and accented, so
